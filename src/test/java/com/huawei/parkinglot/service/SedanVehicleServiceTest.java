...
public class SedanVehicleServiceTest {

    @InjectMocks
    private SedanVehicleService service;

    @Autowired
    private VehicleDAO sedanDAO;
    ...
    public void testCheckIn_trueStory() throws ParseException {
        ...
        sedan = new Vehicle();
        sedan.setLicensePlate("34 AV 34");
        checkInOut.setVehicle(sedan);

        //Act
        service.checkIn(checkInOut);

        //Verify
        Assert.assertNotNull(vehicleDAO.findById(sedan.getLicensePlate()).get());
    }
    ...
    public void testCheckIn_whenCheckInIsAlreadyExist_shouldThrowRestApiRequestExceptionException() {
        ...
        sedan = new Vehicle();
        sedan.setLicensePlate("34 IST 34");
        checkInOut.setVehicle(sedan);
        vehicleDAO.save(checkInOut.getVehicle());
    ...
    public void testCheckIn_whenNotRecordedParkingArea_shouldThrowRestApiRequestExceptionException() {
        ...
        //Act
        sedan = new Vehicle();
        sedan.setLicensePlate("34 AN 34");
        checkInOut.setVehicle(sedan);
    ...
    public void testCheckIn_whenParkingAreaFullCapacity_shouldThrowRestApiRequestExceptionException() {
        //Arrange
        sedan = new Vehicle();
        sedan.setLicensePlate("34 ASD 34");
        checkInOut.setVehicle(sedan);
        parkingArea.setCapacity(1);
        Vehicle sedan2 = new Vehicle();
        sedan2.setLicensePlate("34 ASD 35");
        //Act
        parkingAreaDAO.save(parkingArea);
        vehicleDAO.save(checkInOut.getVehicle());
        checkInOutDAO.save(checkInOut);
    ...
    public void testCheckOut_trueStory() throws ParseException {
        ...
        checkInOut.setParkingArea(parkingAreaRecorded);
        sedan = new Vehicle();
        sedan.setLicensePlate("60 TO 60");
        checkInOut.setVehicle(sedan);
        vehicleDAO.save(sedan);
        checkInOutDAO.save(checkInOut);

        //Act
        service.checkOut(sedan);

        //Verify
        Assert.assertNotNull(checkInOutDAO.findCheckInOutsByVehicleLicensePlate(sedan.getLicensePlate()).get(0).getCheckOutDate());
    }
    ...
    public void testGetParkingDetails_trueStory() throws ParseException {
        ...
        checkInOut.setParkingArea(parkingAreaRecorded);
        sedan = new Vehicle();
        sedan.setLicensePlate("34 TO 34");
        checkInOut.setVehicle(sedan);
        vehicleDAO.save(sedan);
        checkInOutDAO.save(checkInOut);

        //Act & Verify
        Assert.assertNotNull(service.getParkingDetails(sedan.getLicensePlate()));
    }
