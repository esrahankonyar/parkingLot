...
public class SuvVehicleServiceTest {

    @InjectMocks
    private SuvVehicleService service;

    @Autowired
    private VehicleDAO suvDAO;
    ...
    public void testCheckIn_trueStory() throws ParseException {
        ...
        suv = new Vehicle();
        suv.setLicensePlate("34 AV 01");
        checkInOut.setVehicle(suv);

        //Act
        service.checkIn(checkInOut);

        //Verify
        Assert.assertNotNull(suvDAO.findById(suv.getLicensePlate()).get());
    }
    ...
    public void testCheckIn_whenCheckInIsAlreadyExist_shouldThrowRestApiRequestExceptionException() {
        ...
        suv = new Vehicle();
        suv.setLicensePlate("34 IST 02");
        checkInOut.setVehicle(suv);
        suvDAO.save(checkInOut.getVehicle());
    ...
    public void testCheckIn_whenParkingAreaFullCapacity_shouldThrowRestApiRequestExceptionException() {
        //Arrange
        suv = new Vehicle();
        suv.setLicensePlate("34 ASD 04");
        checkInOut.setVehicle(suv);
        parkingArea.setCapacity(1);
        Vehicle sedan2 = new Vehicle();
        sedan2.setLicensePlate("34 ASD 05");
        //Act
        parkingAreaDAO.save(parkingArea);
        suvDAO.save(checkInOut.getVehicle());
        checkInOutDAO.save(checkInOut);
    ...
    public void testCheckOut_trueStory() throws ParseException {
        ...
        checkInOut.setParkingArea(parkingAreaRecorded);
        suv = new Vehicle();
        suv.setLicensePlate("60 TO 06");
        checkInOut.setVehicle(suv);
        suvDAO.save(suv);
        checkInOutDAO.save(checkInOut);

        //Act
        service.checkOut(suv);

        //Verify
        Assert.assertNotNull(checkInOutDAO.findCheckInOutsByVehicleLicensePlate(suv.getLicensePlate()).get(0).getCheckOutDate());
    }
    ...
    public void testGetParkingDetails_trueStory() throws ParseException {
        ...
        checkInOut.setParkingArea(parkingAreaRecorded);
        suv = new Vehicle();
        suv.setLicensePlate("34 TO 07");
        checkInOut.setVehicle(suv);
        suvDAO.save(suv);
        checkInOutDAO.save(checkInOut);

        //Act & Verify
        Assert.assertNotNull(service.getParkingDetails(suv.getLicensePlate()));
    }
